# -*- coding: utf-8 -*-
"""
The following program traces the marginal ray through an optical system in air using the paraxial approximation model. 
...
The program calculates the spherical aberration introduced by each surface in the system, as well as the total spherical 
aberration of the given optical system configuration. 
...
libraries need to be imported (as of December 2017).
"""
import numpy as np
import math
...
comaArray = np.zeros((surfaces,1)) # array to store spherical aberration introduced at each surface
...
for istep in range(0,len(radiusArray)):
    if istep == 0:
        rayHeightArray[istep] = rayHeightArray[istep] # Height of ray on the first surface,4 as defined by the user
