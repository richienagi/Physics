...
The program calculates the distortion introduced by each surface in the system, as well as the total distortion of the given 
optical system configuration. 

Notes: 
- The program assumes that the aperture stop is located at the first lens surface.
- To simulate a planar surfaces, use a large (>500000 mm) radius of curvature.
...
astigmatismArray = np.zeros((surfaces,1)) # array to store the astigmatism introduced at each surface
petzvalCurvatureArray = np.zeros((surfaces,1)) # array to store petzval curvature of each lens surface
distortionArray = np.zeros((surfaces,1)) # array to store the distortion introduced at each surface
...
# Calculates the astigmatism introduced at each surface. Assumes medium before the first surface and after the last surface is 
# air.  
for istep in range(0,len(radiusArray)):
    if istep == 0:
        astigmatismArray[istep] = math.pow(slopeArrayC[istep]/1,2)*sphericalaberrationArray[istep]
    else:
        astigmatismArray[istep] = math.pow((slopeArrayC[istep-1]/indexArray[istep-1]),2)*sphericalaberrationArray[istep]

# Calculates the petzval curvature at each surface 
for istep in range(0,len(radiusArray)):
    if istep == 0:
        petzvalCurvatureArray[istep] = ((1-indexArray[istep])/(1*indexArray[istep]))*curvatureArray[istep]
    elif istep == len(radiusArray)-1:
        petzvalCurvatureArray[istep] = ((indexArray[istep-1]-1)/(1*indexArray[istep-1]))*curvatureArray[istep]
    else:
        petzvalCurvatureArray[istep] = ((indexArray[istep-1]-indexArray[istep])/(indexArray[istep-1]*indexArray[istep]))*curvatureArray[istep]

# Calculates the distortion introduced at each surface
opticalInvariant = (rayHeightArrayC[0]*slopeArray[0]) - (rayHeightArray[0]*slopeArrayC[0])
for istep in range(0,len(radiusArray)):
    if istep == 0:
        distortionArray[istep] = (slopeArrayC[istep]/incidenceAngle)*((math.pow(opticalInvariant,2)*petzvalCurvatureArray[istep]) + astigmatismArray[istep])
    else:
        distortionArray[istep] = ((slopeArrayC[istep-1]/indexArray[istep-1])/(slopeArray[istep-1]/indexArray[istep-1]))*((math.pow(opticalInvariant,2)*petzvalCurvatureArray[istep]) + astigmatismArray[istep])
...
totalDistortion = 0 #Initialize variable for storing sum of distortion aberration contributed by each surface
# prints out distortion contributed by each surface, as well as total distortion of the optical system
for istep in range(0,len(astigmatismArray)):
    totalDistortion = totalDistortion + distortionArray[istep]
    print("The distortion contributed by surface ",istep+1, " is ",distortionArray[istep])
    
print("The total distortion of the system is " ,totalDistortion)
